...
export const TOAST_MESSAGES = {
  ...
  GOAL: {
    CREATED: {
      TITLE: 'Meta criada',
      MESSAGE: (name: string, target: string) => `Meta "${name}" de ${target} foi criada com sucesso`
    },
    CONTRIBUTION: {
    ...
    ERROR: {
      ...
      MESSAGE: (name: string, error?: string) => error || `Não foi possível processar a meta "${name}"`
    }
  },
  ...
  BUDGET: {
    CREATED: {
      ...
      MESSAGE: (category: string, limit: string) => `Orçamento para "${category}" de ${limit} foi criado com sucesso`
    },
    DELETED: {
    ...
    ERROR: {
      ...
      MESSAGE: (category: string, error?: string) => error || `Não foi possível processar o orçamento de "${category}"`
    }
  },
  ...
  CATEGORY: {
    ...
    DELETED: {
      ...
      MESSAGE: (name?: string) => name ? `"${name}" foi excluído com sucesso` : 'Categoria foi excluída com sucesso'
    },
    ERROR: {
