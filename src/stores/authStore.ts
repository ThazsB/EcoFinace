...
export const useAuthStore = create<AuthState>((set, get) => {
  ...
  const activeProfile = activeProfileId ? profiles.find((p: Profile) => p.id === activeProfileId) : null;
  ...
  return {
    ...
    createProfile: async (name, password, avatar = '👤', color = '#F4A261') => {
      ...
      try {
        ...
        localStorage.setItem(`ecofinance_${newProfile.id}_transactions`, JSON.stringify(initialData.transactions));
        localStorage.setItem(`ecofinance_${newProfile.id}_budgets`, JSON.stringify(initialData.budgets));
        localStorage.setItem(`ecofinance_${newProfile.id}_goals`, JSON.stringify(initialData.goals));
        localStorage.setItem(`ecofinance_${newProfile.id}_categories`, JSON.stringify(initialData.categories));
